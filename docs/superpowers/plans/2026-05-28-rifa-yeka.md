...
export function middleware(req: NextRequest) {
  ...
  // Detailed signature verification happens in the page (Node runtime).
...
Note: middleware runs on the Edge runtime and doesn't have access to `node:crypto`. We do cheap presence-check here, and the actual signature verification happens in the page server component / server action via `verifySessionCookie`. Anyone who forges a cookie value gets past the middleware but is rejected by the page.
