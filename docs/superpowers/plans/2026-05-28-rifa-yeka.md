...
export function buildPagueUrl({ name, numbers }: { name: string; numbers: number[] }): string {
  ...
  const sorted = [...numbers].sort((a, b) => a - b).map(pad2).join(', ');
  const text = `Hola Yeka! Ya pagué la rifa. Mis números son ${sorted}. Nombre: ${name}. Adjunto comprobante.`;
...
import { buildPagueUrl, pad2 } from '@/lib/whatsapp';
...
export default async function PagoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const whatsappUrl = buildPagueUrl({ name: purchase.name, numbers });
